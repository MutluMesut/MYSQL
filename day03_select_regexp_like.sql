...
-- Q1 : Icerisinde 'ot' veya 'at' bulunan kelimeleri case-sensitive'e dikkat ederek listele
   ...
   --  [a-zA-Z0-9] = Sadece harf ve rakamlari temsil eder
    
SELECT kelime 
FROM kelimeler 
WHERE REGEXP_LIKE(kelime, 'h[!-~]t','c');-- ikinci harf icin butun karakterleri temsil eder

SELECT kelime 
FROM kelimeler 
WHERE REGEXP_LIKE(kelime, 'h[a-zA-Z0-9]t','c');-- 2. harf icin harf ve rakamlari temsil eder
SELECT kelime 
FROM kelimeler 
WHERE REGEXP_LIKE(kelime, 'h[A-Z]t','c'); -- 2. harf icin sadece A'dan Z'ye olan karakterleri siralar
...
-- Q6 : 2. harfi O olan 3 harfli kelimeleri (o harfi buyuk olanlari) ve id leri listeleyiniz
