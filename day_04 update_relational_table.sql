...
CREATE TABLE urun(
tedarikci_id INT,     -- fk nin data tipi ile pk nin data tipi YANİ ollmali 
urun_id int,
...
-- Irtibat ismi merve temiz olan kaydın sirket ismini getirin
...
set grade=( select yazili_notu
             from puanlar
             where cocuklar.id=puanlar.ogrenci_id); -- birbirine baglı fieldler kullaniliyo
-- where id; -- sadece bir tane recordla ilgili degisiklik olmadigi icin pk olan id yi sectik
		 --  bu son kisim olmadan da islem gerceklesiyor
...
-- 3) Ismi A ile baslayanlarim  urun_fiyatlarini %15 artirin 
